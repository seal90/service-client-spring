...
public class PayloadMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	private Mono<Object> decodeContent(MethodParameter parameter, Message<?> message,
		...
		if(targetType.isArray()
				&& Context.RpcMetadata.class.equals(targetType.getComponentType().resolve())) {
			Context.RpcRequest rpcRequest = (Context.RpcRequest)message.getHeaders().get(CONTEXT_REQUEST_METADATA_KEY);
			return Mono.just(rpcRequest.getMetadataMap().entrySet().stream().map(e -> {
				Context.RpcMetadata rpcMetadata = Context.RpcMetadata.newBuilder().setKey(e.getKey()).setValue(e.getValue()).build();
				return rpcMetadata;
			}).collect(Collectors.toList()).toArray(new Context.RpcMetadata[]{}));
		}

		ResolvableType returnType = ResolvableType.forMethodReturnType(targetMethod);
		Class<?> resolvedReturnType = returnType.resolve();
		if(Flux.class.isAssignableFrom(resolvedType) && Flux.class.isAssignableFrom(resolvedReturnType)) {
			Class<?> parameterGenericType = ResolvableType.forMethodParameter(targetMethod, 0).getGeneric(0).resolve();
			Class<?> returnGenericType = ResolvableType.forMethodReturnType(targetMethod).as(Flux.class).getGeneric(0).resolve();
			if(Request.class.equals(parameterGenericType) && Response.class.equals(returnGenericType)) {

				Class<? extends com.google.protobuf.Message> dataGenericType = (Class<? extends com.google.protobuf.Message>)ResolvableType.forMethodParameter(targetMethod, 0).getGeneric(0).getGeneric(0).resolve();

				ResolvableType contextTargetType = ResolvableType.forClass(Context.Request.class);
				Class<?> contextResolvedType = targetType.resolve();
				ReactiveAdapter contextAdapter = getAdapterRegistry().getAdapter(Context.Request.class);
				ResolvableType contextElementType = (contextAdapter != null ? contextTargetType.getGeneric() : contextTargetType);

				ProtobufDecoder decoder = new ProtobufDecoder();
				Flux<?> flux = content.filter(this::nonEmptyDataBuffer).map(buffer -> decoder.decode(buffer, contextElementType, mimeType, hints))
						.index().map(tuple -> {
							long index = tuple.getT1();
							Context.Request request = (Context.Request)tuple.getT2();
							Any data = request.getData();
                            try {
								com.google.protobuf.Message dataWithType = data.unpack(dataGenericType);
								Map<String, Any> metadata = null;
								if(index == 0) {
									Context.RpcRequest rpcRequest = (Context.RpcRequest)message.getHeaders().get(CONTEXT_REQUEST_METADATA_KEY);
									metadata = rpcRequest.getMetadataMap();
								}
								Request dealRequest = Request.of(metadata, dataWithType);
								return dealRequest;
		...
		if(Mono.class.isAssignableFrom(resolvedType) && Mono.class.isAssignableFrom(resolvedReturnType)) {
			Class<?> parameterGenericType = ResolvableType.forMethodParameter(targetMethod, 0).getGeneric(0).resolve();
			Class<?> returnGenericType = ResolvableType.forMethodReturnType(targetMethod).as(Mono.class).getGeneric(0).resolve();
			if(MessageRequest.class.equals(parameterGenericType) && MessageResponse.class.equals(returnGenericType)) {

				Class<? extends com.google.protobuf.Message> dataGenericType = (Class<? extends com.google.protobuf.Message>)ResolvableType.forMethodParameter(targetMethod, 0).getGeneric(0).getGeneric(0).resolve();

				ResolvableType contextTargetType = ResolvableType.forMethodParameter(targetMethod, 0).getGeneric(0);
				Class<?> contextResolvedType = targetType.resolve();
				ReactiveAdapter contextAdapter = getAdapterRegistry().getAdapter(parameterGenericType);
				ResolvableType contextElementType = (contextAdapter != null ? contextTargetType.getGeneric() : contextTargetType);

				ProtobufDecoder decoder = new ProtobufDecoder();
				Flux<?> flux = content.filter(this::nonEmptyDataBuffer).map(buffer -> decoder.decode(buffer, contextElementType, mimeType, hints))
						.map(decodedMessage -> {
							MessageRequest request = (MessageRequest)decodedMessage;
							Any data = request.getData();
							Context.RpcRequest rpcRequest = (Context.RpcRequest)message.getHeaders().get(CONTEXT_REQUEST_METADATA_KEY);
							if(rpcRequest != null){
								request.putAllMetadata(rpcRequest.getMetadata());
							}
							return request;
						});
