...
final class DefaultRSocketRequester implements RSocketRequester {
	...
	private class DefaultRequestSpec implements RequestSpec {
		...
		@Override
		@SuppressWarnings("unchecked")
		public <T> Flux<Tuple2<T, Map<String, Object>>> retrieveMetadataFlux(Class<T> dataType) {
			...
			ResolvableType elementType = ResolvableType.forClass(dataType);
			if (isVoid(elementType)) {
			...
			return payloadFlux.transformDeferred(flux ->
				flux.index()
					.concatMap(tuple -> {
						Payload payload = tuple.getT2();
						long index = tuple.getT1();

						if (index == 0) {
							Map<String, Object> metadataMap = metadataExtractor.extract(payload, metadataMimeType);
							DataBuffer dataBuffer = retainDataAndReleasePayload(payload);
							@SuppressWarnings("unchecked")
							T data = (T) decoder.decode(dataBuffer, elementType, dataMimeType, EMPTY_HINTS);
							return Mono.just(Tuples.of(data, metadataMap));
						} else {
							DataBuffer dataBuffer = retainDataAndReleasePayload(payload);
							@SuppressWarnings("unchecked")
							T data = (T) decoder.decode(dataBuffer, elementType, dataMimeType, EMPTY_HINTS);
							return Mono.just(Tuples.of(data, Map.<String, Object>of()));
						}
					})
			);
		}
		...
		private <T> Flux<T> retrieveFlux(ResolvableType elementType) {
			...
			if(Payload.class.isAssignableFrom(elementType.resolve())) {
				return (Flux<T>)payloadFlux;
			}
