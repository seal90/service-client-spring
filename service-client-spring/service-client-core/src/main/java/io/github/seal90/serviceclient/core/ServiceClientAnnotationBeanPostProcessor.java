...
public class ServiceClientAnnotationBeanPostProcessor implements BeanPostProcessor, ApplicationContextAware {
  ...
  protected <T> T processInjectionPoint(final Member injectionTarget, final Class<T> injectionType,
    ...
    if(protocol.isEmpty()) {
      protocol = applicationContext.getBean(ServiceClientProperties.class).getDefaultProtocol();
    }
    if(protocol.isEmpty()) {
    ...
    // TODO Looking for a better mechanism to avoid repeatedly creating beans.
    String cacheKeyContent = protocol + ":" + injectionType.getName() + ":" + annotation.serviceName() + ":"
        + annotation.channelName() + ":" + String.join(":", annotation.interceptors());
    String cacheKey = MD5Util.md5Hash(cacheKeyContent);

    @SuppressWarnings("unchecked")
    T bean = (T)cacheBeans.computeIfAbsent(cacheKey, k -> {
      ProtocolTypeFactory factory = findFactory(finalProtocol);
      if(factory != null) {
        return factory.create(injectionTarget, injectionType, annotation);
      }

      throw new IllegalArgumentException("Unsupported protocol: " + finalProtocol);
    });
    return bean;
  }
